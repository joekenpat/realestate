            ...
            <tr>
              ...
              <td>
                <div>
                  ...
                  <a onclick="confirm_action(event, this)" href="{{route('admin_verify_user',['user_id'=>$user->id])}}"
                    style="color:green" uk-tooltip="Verify User" class="uk-icon-link uk-margin-small-right"
                    uk-icon="icon:check; ratio:1"></span></a>
                  @else
                  <a onclick="confirm_action(event, this)"
                    href="{{route('admin_unverify_user',['user_id'=>$user->id])}}" style="color:red"
                    uk-tooltip="Unverify User" class="uk-icon-link uk-margin-small-right"
                    uk-icon="icon:close; ratio:1"></span></a>
                  @endif
                  @if($user->status == 'active')
                  <a onclick="confirm_action(event, this)" href="{{route('admin_block_user',['user_id'=>$user->id])}}" style="color:red" uk-tooltip="Block User"
                    class="uk-icon-link uk-margin-small-right" uk-icon="icon:ban; ratio:1"></span></a>
                  @else
                  <a onclick="confirm_action(event, this)" href="{{route('admin_unblock_user',['user_id'=>$user->id])}}" style="color:green" uk-tooltip="Unblock User"
                    class="uk-icon-link uk-margin-small-right" uk-icon="icon:refresh; ratio:1"></span></a>
                  @endif
